  ...
  try {
    connection.query(
      ...
      (err, result) => {
        ...
        connection.query(
          OrderQuery,
          [orderModel.userID, orderModel.orderDate, orderModel.totalPrice],
          (err, result) => {
            if (err) {
            ...
            connection.query(checkWeaponsQuery, [weaponIDs], (err, results) => {
              ...
              const invalidWeapons = weaponIDs.filter(
                (id) => !existingWeaponIDs.includes(id)
              );
              ...
              if (invalidWeapons.length > 0)
                return res
                  .status(400)
                  .json({
                    message: `Invalid weapon IDs: ${invalidWeapons.join(", ")}`,
                  });
